...
class TestAstype:
    ...
    def test_astype_different_fill_value_keeps_values(self):
        ...
        tm.assert_numpy_array_equal(result.to_dense(), np.array([1, 1, 0, 1]))
    ...
    def test_astype_fill_value_absent_from_values(self):
        ...
        tm.assert_numpy_array_equal(result.to_dense(), np.array([0, 0, 1]))
...
def test_astype_frame_different_fill_value_round_trip():
    ...
    sdf = df.astype(pd.SparseDtype(int, fill_value=1))
    result = sdf.astype(pd.SparseDtype(int, fill_value=0))

    tm.assert_frame_equal(result.astype(int), df)
