...
- Bug in :class:`SparseArray` with a datetime64 or timedelta64 subtype and a :class:`Timestamp` or :class:`Timedelta` ``fill_value`` where ``np.asarray`` and :meth:`SparseArray.astype` returned ``object`` dtype holding raw integers, and :meth:`SparseArray.to_dense`, :meth:`SparseArray.unique`, and :meth:`SparseArray.value_counts` truncated a sub-microsecond ``fill_value``; the equivalent ``np.datetime64``/``np.timedelta64`` spelling was correct (:issue:`68578`)
