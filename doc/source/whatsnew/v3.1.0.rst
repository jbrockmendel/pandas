...
- Bug in :meth:`Series.astype`, :meth:`DataFrame.astype`, :meth:`Index.astype`, and :meth:`SparseArray.astype` from a :class:`SparseDtype` to a dense dtype silently rounding ``uint64`` values above ``2**53``, and, for a datetime64 or timedelta64 subtype with a boxed :class:`Timestamp` or :class:`Timedelta` ``fill_value``, silently giving wrong values or raising ``TypeError`` (:issue:`68506`)
