...
- Performance improvement in :func:`read_csv` with ``engine="c"`` when reading in parallel on a machine with many cores: frames with many columns, and files only a little larger than the size at which parallel reading starts, previously got slower as the worker count rose and are now faster (:issue:`66152`)
